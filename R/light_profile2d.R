...
#' @param breaks Cut breaks for numeric \code{v}. Can be a list of breaks (one element per v).
#' @param n_bins Maxmium number of unique values to evaluate for numeric \code{v}. Only used if neither \code{breaks}, \code{pd_grid} nor \code{pd_evaluate_at} is specified. Can be a vector or list with two elements (one for each v).
#' @param cut_type For the default "equal", bins of equal width are created for each variable in \code{v}. Choose "quantile" to create quantile bins. Can be a vector or list with two elements (one for each v).
#' @param use_linkinv Should retransformation function be applied? Default is TRUE. Not used for type "shap".
#' @param pd_evaluate_at Either a vector or a list with two vectors of evaluation points. Only relevant for type = "partial dependence".
#' @param pd_grid A \code{data.frame} with grid for exactly two columns, e.g. generated by \code{expand.grid}. Only used for type = "partial dependence".
...
#' @return An object of classes \code{light_profile2d} with the following elements.
#' \itemize{
#'   \item \code{data} A tibble containing results. Can be used to build fully customized visualizations. Some column names are detemined by the corresponding option().
