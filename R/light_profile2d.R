...
#'
#' Different binning options are available, see arguments below. For partial dependence and prediction profiles, "model", "predict_function", linkinv" and "data" are required. For response profiles it is "y", "linkinv" and "data" and for shap profiles it is just "shap". "data" can be passed on the fly.
#'
...
#' @param n_bins Maxmium number of unique values to evaluate for numeric \code{v}. Ignored for variables with specified \code{breaks}. Can be an unnamed vector/list of length 2 to distinguish between v.
#' @param cut_type Should numeric \code{v} be cut into "equal" or "quantile" bins? Ignored for variables with specified \code{breaks}. Can be an unnamed vector/list of length 2 to distinguish between v.
#' @param use_linkinv Should retransformation function be applied? Default is TRUE. Not used for type "shap".
#' @param pd_evaluate_at An named list of evaluation points for one or both variables. Only relevant for type = "partial dependence".
