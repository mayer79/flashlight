...
#' light_profile2d(fl, pd_grid = grid, type = "predicted")
#' light_profile2d(fl, pd_grid = grid, type = "response")
...
light_profile2d.flashlight <- function(x, v = NULL,
  ...
  cuts1 <- auto_cut(data[[v[1]]], breaks = breaks,
                    n_bins = n_bins, cut_type = cut_type, ...)
  cuts2 <- auto_cut(data[[v[2]]], breaks = breaks,
                    n_bins = n_bins, cut_type = cut_type, ...)
  ...
  if (type == "partial dependence") {
    ...
    data[[v[1]]] <- cuts1$data[[if (v_labels) "level" else "value"]]
    data[[v[2]]] <- cuts2$data[[if (v_labels) "level" else "value"]]
  }
