...
#' \code{breaks}, \code{pd_evaluate_at} or \code{pd_grid} in order to avoid empty parts
#' in the plot. A high value of \code{n_bins} might not have the desired effect as it
...
#' variables. Used to overwrite automatic binning via \code{n_bins} and \code{cut_type}.
#' Ignored for non-numeric \code{v}.
#' @param n_bins Approximate number of unique values to evaluate for numeric \code{v}.
#' Can be an unnamed vector of length 2 to distinguish between v.
#' @param cut_type Should numeric \code{v} be cut into "equal" or "quantile" bins?
#' Can be an unnamed vector of length 2 to distinguish between v.
#' @param use_linkinv Should retransformation function be applied?
#' Default is \code{TRUE}. Not used for type "shap".
#' @param counts Should observation counts be added?
#' @param counts_weighted If \code{counts} is TRUE: Should counts be weighted by the
...
#' @param pd_grid An evaluation \code{data.frame} with exactly two columns,
#' e.g., generated by \code{expand.grid}. Only used for type = "partial dependence".
...
#' (will be randomly picked from \code{data}). Only used for type = "partial dependence".
#' @param pd_seed Integer random seed used to select ICE profiles.
#' Only used for type = "partial dependence".
#' @param ... Further arguments passed to \code{cut3}/\code{formatC} in forming
#' the cut breaks of the \code{v} variables. Not relevant for partial dependence profiles.
#' @return An object of class "light_profile2d" with the following elements.
#' \itemize{
#'   \item \code{data} A tibble containing results. Can be used to build fully
#'   customized visualizations. Column names can be controlled by
