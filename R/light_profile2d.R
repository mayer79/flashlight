...
light_profile2d.flashlight <- function(x, v = NULL,
  ...
  if (type == "partial dependence") {
      ...
      for (vv in v) {
        ...
        if (is.null(st$pd_evaluate_at)) {
          if (!is.null(st$breaks) && is.numeric(st$breaks)) {
            ...
            arg_list <- c(list(x = data[[vv]]),
              strategy[[vv]][c("n_bins", "cut_type", "breaks")], ...)
            strategy[[vv]]$pd_evaluate_at <-
              do.call(auto_cut, arg_list)$bin_means
          }
        }
      }
    ...
    for (vv in v) {
      st <- strategy[[vv]]
      data[[vv]] <- auto_cut(
        data[[vv]], n_bins = st$n_bins,
        cut_type = st$cut_type, breaks = st$breaks, ...
      )$data$level
    }
  }
