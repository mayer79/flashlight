...
#'
#' Calculates different types of 2D-profiles across two variables. By default, partial dependence profiles are calculated. Other options are response, predicted values, residuals, and shap. The results are aggregated by (weighted) means.
#'
#' Numeric v with more than \code{n_bins} disjoint values are binned. Alternatively, \code{breaks} can be provided to specify the binning (a single vector or an unnamed list of two vectors). For partial dependence profiles, this behaviour can be overwritten either by providing a single vector (or an unnamed list of two vectors) of evaluation points (\code{pd_evaluate_at}) or an evaluation \code{pd_grid}. For partial dependence and prediction profiles, "model", "predict_function", linkinv" and "data" are required. For response profiles it is "y", "linkinv" and "data" and for shap profiles it is just "shap". "data" can be passed on the fly.
#'
#' @importFrom withr with_options
#' @importFrom stats setNames
#' @param x An object of class \code{flashlight} or \code{multiflashlight}.
#' @param v A vector of exactly two variable names.
...
#' @param breaks Cut breaks for numeric \code{v}. Can be a single vector or an unnamed list of two vectors (in the same order as v).
#' @param n_bins Maxmium number(s) of unique values to evaluate for numeric \code{v}. Only used if neither \code{breaks}, \code{pd_grid} nor \code{pd_evaluate_at} is specified. Can be a vector/list with two elements in the same order as v.
#' @param cut_type For the default "equal", bins of equal width are created for each variable in \code{v} (recommended for plotting). Choose "quantile" to create quantile bins. Can be a vector/list with two elements in the same order as v.
#' @param use_linkinv Should retransformation function be applied? Default is TRUE. Not used for type "shap".
#' @param pd_evaluate_at Either a single vector of evaluation points or an unnamed list of two vectors (in the same order as v). Only relevant for type = "partial dependence".
#' @param pd_grid An evaluation \code{data.frame} with exactly two columns, e.g. generated by \code{expand.grid}. Only used for type = "partial dependence".
...
light_profile2d.flashlight <- function(x, v = NULL,
  ...
  if (type == "partial dependence") {
    ...
    # Replace v values by binned values (ifelse required)
    if (!is.null(breaks)) {
