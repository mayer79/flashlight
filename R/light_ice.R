...
#' The first option is to pass the variable name via `v` and an optional
#' vector with evaluation points `evaluate_at` (or `breaks`).
#' This works for dependence on a single variable. The second option is much more
#' general: You can specify any `grid` as a `data.frame` with one or
...
#' (with potentially undesired consequences for subsequent code) or a vector of
#' indices used to select rows. In both cases, `data` should be the same for
#' all flashlights considered.
#'
...
#' @param center How should curves be centered? Default is "no".
