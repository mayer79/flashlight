...
#'
#' Generates Individual Conditional Expectation (ICE) profiles. A ICE profile shows how the prediction of an observation changes if a variable is systematically changed across its range, see e.g. [1] for reference. The curves can be centered in order to increase visibility of interaction effects. Centering is done within subgroups specified by "by".
#'
...
light_ice.flashlight <- function(x, v = NULL, data = x$data, by = x$by,
  ...
  if (center) {
    ...
    local_shift <- local_shift[match(data[[id_name]], central_data[[id_name]])]
    data[[value_name]] <- data[[value_name]] + local_shift
  }
