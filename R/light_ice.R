...
light_ice.flashlight <- function(x, v = NULL, data = x$data, by = x$by,
                                 ...
                                 n_bins = 27, indices = NULL, n_max = 20,
  ...
  if (is.null(grid)) {
    if (is.null(evaluate_at)) {
      if (is.null(breaks)) {
        evaluate_at <- unique(auto_cut(data[[v]], n_bins = n_bins, ...)[["value"]])
      } else {
        breaks <- sort(unique(breaks))
        stopifnot((nbreaks <- length(breaks)) >= 2L)
        evaluate_at <- (breaks[-nbreaks] + breaks[-1]) / 2
      }
    }
