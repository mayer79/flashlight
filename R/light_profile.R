...
#'
#' Calculates different types of profiles across covariable values. By default, partial dependence profiles [1] are calculated. Other options are profiles of ALE (accumulated local effects, see [2]), response, predicted values ("M plots" or "marginal plots", see [2]) and residuals are possible. The results are aggregated either by (weighted) means or by (weighted) quartiles.
#'
...
light_profile.flashlight <- function(x, v = NULL, data = NULL, by = x$by,
  ...
  if (type == "partial dependence") {
    ...
    agg <- ale_calculation(x, v = v, breaks = breaks, n_bins = n_bins,
                           cut_type = cut_type,
