...
#' @param shap_name Column name in \code{data} of element "shap" containing SHAP values. Defaults to "shap".
...
#' @return An object of class \code{flashlight} or \code{multiflashlight} with additional element "shap" of class "shap".
#' @export
...
add_shap.flashlight <- function(x, v = NULL,
                                ...
                                shap_name = "shap", variable_name = "variable", ...) {
  ...
  stopifnot(length(v) >= 1L,
            ...
            !anyDuplicated(c(x$by, x$w, v, shap_name, variable_name)))
  ...
  core_func <- function(i) {
    ...
    shp[[shap_name]] <- shp[["after_"]] - shp[["before_"]]
    shp[1, shap_name] <- shp[1, "before_"]
    expand_grid(shp[, c(variable_name, shap_name)], new_obs[i, ])
  }
  ...
  # Organize output
  shap <- list(data = bind_rows(out),
               shap_name = shap_name, variable_name = variable_name,
               v = v, by = x$by, w = x$w)
  class(shap) <- "shap"
