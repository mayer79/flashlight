...
2. Calculate relevant information by calling the key functions:
   ...
   - `light_interaction`: Calculates interaction strength based on 0-mean-centered ICE curve scatter.
...
In the same way as diverging ICE profiles give a clou of presence of interactions, we can use the option `stats = "quartiles"` (with `pd_center`) to show divergence of the centered ICE profiles as boxes (predictions at log-scale to suppress interaction-like effects of the retransformation function:
...
Using `light_interaction`, we can investigate which variable has strongest overall interaction effects based on the proportion of variability in c-ICE profiles unexplained by the considered variable.
...
Pairwise interactions are expensive to compute as each variable pair has to be assessed by the help of two-dimensional ICE-profiles. We limit the search for interactions to the top four variables.
```{r}
st_pair <- light_interaction(fls, v = most_important(imp, 4), type = "pairwise") 
plot(st_pair)
```
...
plot(light_profile(fls, v = "sqft_living", stats = "quartiles", center = "middle"))
