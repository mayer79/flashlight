...
Instead of studying global variable importance and effects, there are different techniques to entangle how a *single prediction* can be decomposed into additive effects of input variables. For non-additive models, the order of the variables visited is relevant. The fairest way to create such decomposition would consider all possible orderings (permutations) and averaging the results. This is what SHAP values do, see e.g. Gosiewska and Biecek [4]. Since there is no model agnostic *efficient* way to calculate them, we use an approximation called *breakdown* described in [4]. 

The breakdown algorithm assigns the following attribution to the first variable $x_1$: In the query data, all values in $x_1$ are set to the value of $x_1$ of the considered observation. The change in the (weighted) average predicted value on the query data measures the contribution of $x_1$ on the prediction. This procedure is iterated over further $x_i$. In an initial step, the variables $x_i$ are sorted by the size of their contribution in the same way but without iteration.
...
# Variable contribution breakdown for single observation
plot(light_breakdown(fl_weighted, new_obs = iris[2, ]))
```
...
[4] A. Gosiewska and P. Biecek (2019). IBREAKDOWN: Uncertainty of model explanations for non-additive predictive models. ArXiv <arxiv.org/abs/1903.11420>.
