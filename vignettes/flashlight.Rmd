...
2. It offers model agnostic tools like model performance, variable importance, ICE profiles, partial dependence, further effects plots, variable interaction, and variable contribution breakdown for single observations.
...
Besides measuring overall variable importance, an interesting aspect is to measure the strength of non-additivity associated with each covariable (i.e. the "overall interaction strength") and/or between pairs of covariables. Model-agnostic approaches to measure interaction strength are often based on (zero-mean) centered ICE-curves. How much of their variance is unexplained by the main effect of $X$? This measures the strength of all interactions associated with $X$. To measure pairwise interactions, we can study how much variability of two-dimensional c-ICE curves can be explained by the main effects of the two variables (obtained from one-dimensional c-ICE curves). These unnormalized measures can be compared across variable (pairs) to see absolute interaction strength. Alternatively, the values are standardized with the total variance of the underlying c-ICE profiles. For unnormalized values it makes sense to root-transform the values in order to be on the same scale as the response. For the normalized case, it is easier to interprete the values are proportion of variance explained, so we suggest do not take roots there. If done anyway, for pairwise interactions, we can reproduce Friedman's $H$ statistic in [7]. 
...
2. Calculate relevant information by calling the key functions:
   ...
   - `light_interaction`: Calculates overall and pairwise interaction strength based on 0-mean-centered ICE curve scatter.
...
Using `light_interaction`, we can investigate which variable has strongest overall interaction effects based on the root of the variability of centered ICE profiles unexplained by the main effects.

```{r}
st <- light_interaction(fls, v = x) 
plot(st)
```
Since we have not used interaction terms in the linear model, all values are zero there. On the scale of the response, the strongest interaction effects are in combination with "waterfront".

Pairwise interactions are expensive to compute as each variable pair has to be assessed by the help of two-dimensional ICE-profiles. We limit the search for interactions to the four variables with highest interaction strength.
```{r}
st_pair <- light_interaction(fls, v = most_important(st, 4), type = "pairwise") 
plot(st_pair)
```

Friedman's H statistic [7] is found by normalizing the results and taking roots.

```{r}
st_pair <- light_interaction(fls, v = most_important(st, 4), 
                             type = "pairwise", normalize = TRUE, take_sqrt = TRUE) 
plot(st_pair)
```
