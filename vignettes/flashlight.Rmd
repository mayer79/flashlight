...
Besides measuring overall variable importance, an interesting aspect is to measure the strength of non-additivity associated with each covariable (i.e. the "overall interaction strength") and/or between specific covariables. One way to calculate interaction strength between two covariables is based on the difference between univariate and bivariate partial dependence profiles. Overall interaction strength can be derived by marginalization. A less computationally intensive alternative is based on mean-centered ICE curves: The higher the average within-evaluation-point variance between curves, the stronger the overall interaction strength associated with variable $v$. If $v_j$ denotes the $j$-th evaluation point of $v$ and $x_{i,j}$ the value of the mean-centered ICE curve of the $i$-th profile at $v_j$, then the (unweighted) overall interaction strength of variable $v$ is estimated as
$$
  S(v) = \sqrt{\text{Avg}_j Var_i(x_{i,j})}.
$$
To our knowledge, this measure of interaction strength has not yet been proposed, although other approaches based on derivative ICE curves are available in the literature [3]. An alterantive way to calculate $S(v)$ is based on repeated measures ANOVA with one fixed factor $v$ and a within curve repeated measure factor $i$. $S$ is then estimated by its residual standard deviation.
...
2. Calculate relevant information by calling the key functions:
   ...
   - `light_interaction`: Calculates overall interaction strength based on mean-centered ICE curve scatter.
